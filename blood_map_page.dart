...
class _BloodMapPageState extends State<BloodMapPage> {
  ...
  // Vị trí mặc định (Hà Nội nếu chưa có GPS)
  LatLng _currentPosition = const LatLng(21.028511, 105.804817);
  bool _isLoadingLocation = true;

  // Mock data SOS và Bệnh viện
  ...
  Widget _filters() {
    return Row(
      children: [
        _chip("Chỉ hiện SOS", true),
        const SizedBox(width: 8),
        _chip("Gần nhất", false),
        const SizedBox(width: 8),
        ...
        return Container(
          ...
          child: ListView(
            ...
            children: [
              ...
              // Render SOS Items with dynamic distance
              ..._sosPoints.map((sos) => Padding(
                padding: const EdgeInsets.only(bottom: 12),
                child: _sosItem(sos),
              )),

              // Render Hospital Items with dynamic distance
              ..._hospitalPoints.map((hosp) => Padding(
                padding: const EdgeInsets.only(bottom: 12),
                child: _hospitalItem(hosp),
              )),

              const SizedBox(height: 12),
              const Text(
                "Chúng tôi chỉ hiển thị nhóm máu & địa điểm – bảo mật thông tin cá nhân",
  ...
  Widget _sosItem(Map<String, dynamic> sos) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          const Icon(Icons.warning, color: ctaOrange),
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text(
                  "$distance • ${sos['time']}",
                  style: const TextStyle(color: ctaOrange, fontSize: 12, fontWeight: FontWeight.w600),
                ),
              ],
            ),
          ),
          GestureDetector(
            onTap: () {
              _mapController.move(LatLng(sos['lat'], sos['lng']), 15.0);
            },
            child: Container(
              padding: const EdgeInsets.all(10),
              decoration: BoxDecoration(
                color: ctaOrange.withOpacity(0.15),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          GestureDetector(
             onTap: () {
              _mapController.move(LatLng(hosp['lat'], hosp['lng']), 15.0);
            },
